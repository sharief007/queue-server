...
public readonly struct Message : IDisposable
{
    ...
    public void Serialize(Span<byte> buffer)
    {
        if (buffer.Length < TotalSize)
            throw new ArgumentException(
                $"Buffer too small. Required: {TotalSize}, Available: {buffer.Length}"
            );
    ...
    public static (Message Message, int BytesRead) Deserialize(ReadOnlyMemory<byte> buffer)
    {
        ...
        if (buffer.Length < totalLength)
            throw new ArgumentException(
                $"Buffer too small for complete message. Required: {totalLength}, Available: {buffer.Length}"
            );
...
public sealed class MessageBuilder : IDisposable
{
    ...
    private void SerializeProperties()
    {
        ...
        var estimatedSize = _properties.Sum(kv =>
            Encoding.UTF8.GetByteCount(kv.Key) + Encoding.UTF8.GetByteCount(kv.Value) + 2
        ); // +2 for '=' and '\0'
